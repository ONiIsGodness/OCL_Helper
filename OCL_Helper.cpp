...
OCL_Error OCL_Helper::eStatus = OCL_ERR_INIT;
...
{
    if( (eStatus = Initialize()) != OCL_ERR_OK )
       return OCL_ERR_INIT;
    
    char msg[MSG_BUFFER_SIZE] = { 0 };
    size_t size;
    
    for( const auto& plt : vPlatforms ){
        clGetPlatformInfo(plt.Platform, CL_PLATFORM_NAME, MSG_BUFFER_SIZE, msg, &size);
        msg[size - 1] = ',';
        msg[size] = ' ';
        clGetPlatformInfo(plt.Platform, CL_PLATFORM_VERSION, MSG_BUFFER_SIZE - size - 1, msg + size + 1, &size);
        printf("%s\n", msg);
...
OCL_Error OCL_Helper::Initialize()
{
	...
	vPlatforms.reserve(MAX_OCL_PLATFORMS);
...
ostream&  operator<< ( ostream& os,  const OCL_Helper& ocl)
{
    ...
    if( NULL == ocl.device || NULL == ocl.platform ){
        ...
        msg[size - 1] = ',';
        msg[size] = ' ';
        clGetDeviceInfo(ocl.device, CL_DEVICE_NAME, OCL_Helper::MSG_BUFFER_SIZE - size - 1, msg + size + 1, NULL);
    }
    os << msg;
    return os;
}
...
OCL_Error OCL_Helper::ObtainKernel(const string& cl_file, const string& program, cl_kernel& kernel)
{
    ...
    return CL_SUCCESS == err ? OCL_ERR_OK : OCL_ERR_OBTAIN_KERNEL;
}
...
OCL_Error OCL_Helper::PushTask( const string& cl_file, const string& program, size_t result_size, void* result, cl_mem device_mem, int param_index )
{
    ...
    if( (nullptr != result) && (device_mem != nullptr) )
        err = clEnqueueReadBuffer(queue, device_mem, CL_TRUE, 0, result_size, result, 0, NULL, NULL);
...
OCL_Error OCL_Helper::CreateMemObject(cl_mem memObject[1], void* host_mem, size_t size)
{
	...
	cl_int err;
    memObject[0] = clCreateBuffer(context, flag, size, host_mem, &err);
    if( NULL == memObject[0] )
